...
if (isset($_POST['odoslat'])) {
  ...
  $dat_zaistenia = $_POST['dat_zaistenia'];
  $cvs = $_POST['cvs'];



  if ($barcode OR $cvs OR $dat_zaistenia) {
    ...
    DATE_FORMAT(dat_prijatia, '%d. %m. %Y %H:%i') AS dat_prijatia, DATE_FORMAT(dat_vyradenia, '%d. %m. %Y %H:%i') AS dat_vyradenia, id_ps FROM ps WHERE barcode LIKE '$barcode' OR cvs LIKE '$cvs' OR dat_zaistenia LIKE '$dat_zaistenia'";
    ...
    if ($výsledok) {
      ...
      while ($riadok = mysql_fetch_array($výsledok, MYSQL_NUM)) {
        ...
        <th style=\"font-size:20pt;color:red;\" align=\"left\">$riadok[2]</th></tr>
        ...
        if (isset($_GET['zapis'])) {
          echo '<td align="left"><a href="./zapis-pi.php?pripad=' . $riadok[54];
          if (isset($_GET['osoba'])) {
            echo '&osoba=' . $_GET['osoba'];
          }
...
<fieldset style="height:200px;width:420px;border-radius:5px;border:thin solid forestgreen;" class="fieldset">
  <legend><h3 style="color:forestgreen;text-transform:uppercase">Zadajte parametre vyhľadávania:</h3></legend><br><p>
  <table align="center">
    <tr><td align="left">Podľa čiarového kódu:&nbsp</td>
      <td><input type="text" class="frame" name="barcode" size="20" maxlength="40" style="height:30px;width:225px;font-size:14pt;"
        value="<?php if (isset($_POST['barcode'])) echo $_POST['barcode']; ?>"/></td></tr>
  <tr><td align="left">ČVS:&nbsp</td>
      <td><input type="text" class="frame" name="cvs" size="20" maxlength="40" style="height:30px;width:225px;font-size:14pt;"
        value="<?php if (isset($_POST['cvs'])) echo $_POST['cvs']; ?>"/></td></tr>
  <tr><td align="left">Dátum zaistenia:&nbsp</td>
      <td><input type="date" class="frame" name="dat_zaistenia" size="20" maxlength="40" style="height:30px;width:225px;font-size:14pt;"
        value="<?php if (isset($_POST['dat_zaistenia'])) echo $_POST['dat_zaistenia']; ?>"/></td></tr></table>
</fieldset>
