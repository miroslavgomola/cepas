...
if (isset($_POST['odoslat'])) {
  ...
  if ($barcode OR $cvs OR $dat_zaistenia) {
    ...
    DATE_FORMAT(dat_prijatia, '%d. %m. %Y %H:%i') AS dat_prijatia, DATE_FORMAT(dat_vyradenia, '%d. %m. %Y %H:%i') AS dat_vyradenia FROM ps WHERE barcode LIKE '$barcode' OR cvs LIKE '$cvs' OR dat_zaistenia LIKE '$dat_zaistenia'";
    ...
    if ($výsledok) {
      ...
      echo "<table style=\"font-size:14pt;width:1230px;border:thin solid forestgreen;\" cellspacing=\"0\" cellpadding=\"2\">
    ...
    if ($výsledok) {
      ...
      <a href="./cislo_ps.php">Priradiť</button></p>';
